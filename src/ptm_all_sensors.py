...
from keras.layers import Input, Conv1D, Conv2D, Dense, Flatten,MaxPooling2D, concatenate,Permute,Reshape,LSTM
...
#     """
#     multi_input = Input(shape=(SLIDING_WINDOW_LENGTH,NUM_CHANNEL), name='multi_input')
#     print(multi_input.shape)  # (?, 1, 24, 113)
    
#     y = Conv1D(64, FILTER_SIZE, padding='causal',activation='relu')(multi_input)
#     print(y.shape)  # (?, 64, 20, 113)
    
#     y = Conv1D(64, FILTER_SIZE, padding='causal',activation='relu')(y)
#     print(y.shape)
#     y = Conv1D(64, FILTER_SIZE, padding='causal',activation='relu')(y)
#     print(y.shape)
#     y = Conv1D(64, FILTER_SIZE, padding='causal',activation='relu')(y)
...
plt.savefig('all3sensors_model.png')

# serialize model to JSON
model_json = model.to_json()
with open("4act_all3sensors.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("4act_all3sensors.h5")
