# -*- coding: utf-8 -*-
"""
Created on Thu Mar 18 22:54:22 2021
ankle model
train a model for ankle, use trial1+trial2 data
...
def load_by_trial(trial_id,sensor):
    path = '../segmented/trial'+str(trial_id)+'/*_'+sensor+'_train_data.txt'
    x_list = glob.glob(path)
    path = '../segmented/trial'+str(trial_id)+'/*_'+sensor+'_train_label.txt'
...
plt.savefig('ankle_model.png')

# serialize model to JSON
model_json = model.to_json()
with open("4act_ankle.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("4act_ankle.h5")
